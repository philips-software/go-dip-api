// Package provisioning provides support the DIP HSDP Provisioning services
...
const (
	userAgent  = "go-hsdp-api/provisioning/" + internal.LibraryVersion
	APIVersion = "1"
)
...
// A Client manages communication with DIP HSDP Provisioning APIs
type Client struct {
	...
	*iam.Client
	...
	OrgConfigurationsService *OrgConfigurationsService
}

// NewClient returns a new provisioning client
func NewClient(iamClient *iam.Client, config *Config) (*Client, error) {
	...
	c.OrgConfigurationsService = &OrgConfigurationsService{Client: c, validate: validator.New()}
...
func doAutoconf(config *Config) {
	if config.Region != "" && config.Environment != "" {
		...
		if err == nil {
			theService := c.Service("provisioning")
...
func (c *Client) NewRequest(method, requestPath string, opt interface{}, options ...OptionFunc) (*http.Request, error) {
	...
	if opt != nil {
		...
		u.RawQuery = strings.ReplaceAll(q.Encode(), "+", "%20") // https://github.com/golang/go/issues/4013
	}
...
// Response is a HSDP provisioning API response. This wraps the standard http.Response
// returned from HSDP provisioning and provides convenient access to things like errors
